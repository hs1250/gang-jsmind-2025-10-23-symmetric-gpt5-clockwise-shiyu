...
function applySymmetricLayout(root){
  if(!root || !root.children || root.children.length===0) return;
  const kids = root.children.slice();
  const half = Math.ceil(kids.length/2);
  const right = kids.slice(0, half);      // first half -> right, keep order (top->bottom)
  const left = kids.slice(half).reverse(); // second half -> left, reverse for clockwise layout
  right.forEach(n => { n.direction = 'right'; });
  left.forEach(n => { n.direction = 'left'; });
...
function parseMarkdown(mdText) {
  ...
  lines.forEach(line => {
    ...
    if(parent && parent.direction){ node.direction = parent.direction; }
    stack[stack.length - 1].children.push(node);
    stack.push(node);
  });
  // Make root children expand symmetrically like XMind and order clockwise by MD order
  applySymmetricLayout(root);
...
document.addEventListener('keydown',(e)=>{
  ...
  document.getElementById('btn_open_cloud').onclick = () => {
    ...
    fetch(`${gasUrl}?action=getList`)
      ...
      .catch(err => {
        cloudFilesList.innerHTML = `<li>讀取失败: ${err.message}</li>`;
        allCloudFiles = [];
      });
  };
